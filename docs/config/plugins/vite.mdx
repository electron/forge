...
The main process and preload bundles are built as Node targets rather than browser bundles, so Node globals such as `__dirname` and `import.meta.url` compile to working CommonJS, and `process.env` is read at runtime instead of being inlined at build time.
