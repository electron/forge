...
While `electron-forge start` is running, the plugin adds a tab for every Vite
build (each `build` target, such as the main process and preload scripts) and
for every renderer dev server (its requests, HMR updates and errors) to Forge's
...
`q` (or `Ctrl+C`) to quit. When a build fails, the view switches to its tab
automatically. When the terminal is not interactive, or `CI` is set, the same
output is written as plain lines prefixed with the tab name instead.
