...
The plugin derives Vite's [`build.target`](https://vite.dev/config/build-options.html#build-target) from the Electron version installed in your project, so your code is only downleveled for the engines Electron actually runs. Main process and preload bundles are built for the Node.js version that Electron ships (for example `node22.20`), and renderer bundles are built for the Chromium version it ships (for example `chrome142`). If the Electron version cannot be determined, Vite's own default target is used instead.
