...
The plugin tweaks a few Vite defaults for renderers before merging in your own config, which always wins:

* `base` is set to `'./'` for production builds only, because the packaged app loads `index.html` off disk with `loadFile`. The dev server is served over HTTP, so it keeps Vite's default `/` base.
* `build.modulePreload.polyfill` is turned off, since Electron's Chromium supports `<link rel="modulepreload">` natively.
* `build.reportCompressedSize` is turned off for renderer, main and preload builds, since gzip sizes say nothing about code shipped inside an asar.

Set any of them explicitly in your renderer's Vite config to override the default:
