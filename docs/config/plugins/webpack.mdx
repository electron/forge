...
While `electron-forge start` is running, the plugin renders the output of every
webpack compiler (main process, each renderer and preload target) and of the
Electron app itself (the `App` tab) as tabs in the terminal. Use `←`/`→` or the
tab numbers to switch tabs, `a` for a merged view, `↑`/`↓` to scroll back, `r`
to restart the Electron app and `q` (or `Ctrl+C`) to quit. When a compiler
fails, the view switches to its tab automatically (at most once every 15
seconds). When the terminal is not interactive, or `CI` is set, the same output
is written as plain lines prefixed with the tab name instead.
