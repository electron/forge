...
First, your `main` entry in your `package.json` file needs to point at `"./.webpack/main/index.cjs"` like so:
...
The main process and preload bundles are emitted with a `.cjs` extension, so Electron parses them as CommonJS even if your `package.json` has `"type": "module"`. The `main` field has to name the file explicitly because Node's directory lookup only finds `index.js`.
:::
...
While `electron-forge start` is running, the plugin adds a tab for every
webpack compiler (main process, each renderer and preload target) to Forge's
[terminal UI](../../cli.md#terminal-ui), next to the `App` tab that shows the
Electron app's own output. Use `←`/`→` or the tab numbers to switch tabs, `a`
for a merged view, `↑`/`↓` to scroll back, `r` to restart the Electron app and
`q` (or `Ctrl+C`) to quit. When a compiler fails, the view switches to its tab
automatically (at most once every 15 seconds). When the terminal is not
interactive, or `CI` is set, the same output is written as plain lines prefixed
with the tab name instead.
...
For the main process, press `r` in the compiler output view (or type `rs` when the interactive view is not active) in the console you launched `electron-forge` from and Forge will restart your app for you with the new main process code.
