...
Electron app itself as tabs in the terminal. Use `←`/`→` or the tab numbers to
switch tabs, `a` for a merged view, `↑`/`↓` to scroll back, `r` to restart the
Electron app and `q` (or `Ctrl+C`) to quit. When the terminal is not
interactive, or `CI` is set, the same output is written as plain lines
prefixed with the tab name instead.
