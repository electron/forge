...
The publisher uploads `RELEASES` and `RELEASES.json` only after every other artifact has finished uploading, so an app checking for updates never sees a release whose files are not there yet.
