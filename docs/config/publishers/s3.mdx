...
The publisher uploads `RELEASES` and `RELEASES.json` only after the other artifacts for the same platform and arch have finished uploading, so an app checking for updates never sees a release whose files are not there yet.
