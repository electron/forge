...
export const createCompileHook = (originalDir: string) => async (_config: ForgeConfig, buildPath: string): Promise<void> => {
      ...
      for (const entry of await fs.readdir(appDir)) {
        if (!entry.match(/^(node_modules|bower_components)$/)) {
          ...
          if ((await fs.stat(fullPath)).isDirectory()) {
            const { log } = console;
            console.log = () => { /* disable log function for electron-compile */ };
