...
import { Compiler, Configuration, MultiStats, rspack, Stats, Watching } from '@rspack/core';
import { RspackDevServer, Configuration as RspackDevServerConfiguration } from '@rspack/dev-server';
...
import { RspackPluginConfig, RspackPluginRendererConfig } from './Config';
import RspackConfigGenerator from './RspackConfig';
import ElectronForgeLoggingPlugin from './utils/ElectronForgeLogging';
import EntryPointPreloadPlugin from './utils/EntryPointPreloadPlugin';
import once from './utils/once';

type RspackToJsonOptions = Parameters<Stats['toJson']>[0];
type RspackWatchHandler = Parameters<Compiler['watch']>[1];
...
export default class RspackPlugin extends PluginBase<RspackPluginConfig> {
  ...
  get configGenerator(): RspackConfigGenerator {
    if (!this._configGenerator) {
      this._configGenerator = new RspackConfigGenerator(this.config, this.projectDir, this.isProd, this.port);
    }
    ...
    return {
      prePackage: [
        namedHookWithTaskFn<'prePackage'>(async (task, config, platform, arch) => {
          if (!task) {
            throw new Error('Incompatible usage of rspack-plugin prePackage hook');
          }
              ...
              : [
                  {
                    ...
                    task: async (ctx: NativeDepsCtx) => {
                      ...
                      const rspackNodeFiles = await glob('**/*.node', {
                        cwd: firstArchDir,
                      });
                      ...
                      for (const rspackNodeFile of rspackNodeFiles) {
                        const hash = crypto.createHash('sha256');
                        await pipeline(fs.createReadStream(path.resolve(firstArchDir, rspackNodeFile)), hash);
                        const matchedNodeModule = hashToNodeModules[hash.digest('hex')];
                        if (!matchedNodeModule || !matchedNodeModule.length) {
                          throw new Error(`Could not find originating native module for "${rspackNodeFile}"`);
                        }

                        mapping[rspackNodeFile] = matchedNodeModule;
                      }
      ...
      packageAfterCopy: [
        async (_forgeConfig: ResolvedForgeConfig, buildPath: string, _electronVersion: string, _platform: string, pArch: string): Promise<void> => {
          // Restore the correct 'arch' build of rspabk
          // Steal the correct arch, wipe the folder, move it back to pretend to be "dist" root
          const tmpRspackDir = path.resolve(buildPath, 'dist.tmp');
          await fs.move(path.resolve(buildPath, 'dist', pArch), tmpRspackDir);
          await fs.remove(path.resolve(buildPath, 'dist'));
          await fs.move(tmpRspackDir, path.resolve(buildPath, 'dist'));
        },
  ...
  private async compileMain(watch = false, logger?: Logger): Promise<void> {
    let tab: Tab;
    if (logger) {
    ...
    await new Promise((resolve, reject) => {
      const compiler = rspack(mainConfig);
      ...
      const cb: RspackWatchHandler = async (err, stats) => {
        ...
        if (this.config.jsonStats) {
          await this.writeJSONStats('main', stats, mainConfig.stats as RspackToJsonOptions, 'main');
        }
  ...
  private async compileRenderers(watch = false): Promise<void> {
    for (const rendererOptions of this.allRendererOptions) {
      const stats = await this.runRspack(await this.configGenerator.getRendererConfig(rendererOptions), rendererOptions);
      if (!watch && stats?.hasErrors()) {
        throw new Error(`Compilation errors in the renderer: ${stats.toString(undefined)}`);
      }
    }
  }

  private async runRspack(options: Configuration[], rendererOptions: RspackPluginRendererConfig | null): Promise<MultiStats | undefined> {
    return new Promise((resolve, reject) => {
      rspack(options).run(async (err, stats) => {
        if (rendererOptions && rendererOptions.jsonStats) {
          for (const [index, entryStats] of (stats?.stats ?? []).entries()) {
            ...
            await this.writeJSONStats('renderer', entryStats, options[index].stats as RspackToJsonOptions, name);
          }
        }
        if (err) {
          return reject(err);
  ...
  private setDirectories(dir: string) {
    this.projectDir = dir;
    this.baseDir = path.resolve(dir, 'dist');
  }

  private async launchRendererDevServers(logger: Logger): Promise<void> {
    ...
    for (const [i, rendererOptions] of this.allRendererOptions.entries()) {
      ...
      const rendererConfig = await this.configGenerator.getRendererConfig(rendererOptions);
      configs.push(
        ...rendererConfig.map((config) => ({
          ...config,
          name: groupName,
    ...
    const promises = preloadPlugins.map((preloadPlugin) => {
      return new Promise((resolve, reject) => {
        compiler.hooks.done.tap(preloadPlugin, (stats) => {
          if (stats.hasErrors()) {
            return reject(new Error(`Compilation errors in the preload: ${stats.toString(undefined)}`));
          }
    ...
    const rspackDevServer = new RspackDevServer(this.devServerOptions(), compiler);
  ...
  private devServerOptions(): RspackDevServerConfiguration {
    ...
    const overrides: Partial<RspackDevServerConfiguration> = {
      port: this.port,
  ...
  private async writeJSONStats(type: string, stats: Stats | undefined, statsOptions: RspackToJsonOptions, suffix: string): Promise<void> {
    if (!stats) return;
  ...
  private async resolveForgeConfig(forgeConfig: ResolvedForgeConfig): Promise<ResolvedForgeConfig> {
...
Your packaged app may be larger than expected if you dont ignore everything other than the 'dist/' folder`)
        );
      }
      return forgeConfig;
    }
    forgeConfig.packagerConfig.ignore = (file: string) => {
      ...
      if (this.config.jsonStats && file.endsWith(path.join('dist', 'main', 'stats.json'))) {
        return true;
      }

      if (this.allRendererOptions.some((r) => r.jsonStats) && file.endsWith(path.join('dist', 'renderer', 'stats.json'))) {
        return true;
      }
  ...
  private async packageAfterCopy(_forgeConfig: ResolvedForgeConfig, buildPath: string): Promise<void> {
    const pj = await fs.readJson(path.resolve(this.projectDir, 'package.json'));

    if (!pj.main?.endsWith('dist/main')) {
      throw new Error(`Electron Forge is configured to use the Rspack plugin. The plugin expects the
"main" entry point in "package.json" to be "dist/main" (where the plugin outputs
