...
export interface ParsedInitOptions {
  dir: string;
  template: string;
  copyCIFiles: boolean;
  force: boolean;
...
export interface ParsedImportOptions {
  dir: string;
  skipGit: boolean;
}

export function getInitOptions(argv: string[]): ParsedInitOptions {
  let dir: string;
  ...
  const initCommand = program
    ...
    .action((targetDir) => {
      dir = resolveWorkingDir(targetDir, false);
    });

  // Register the import subcommand so commander doesn't error on `import`
  program
    .command('import')
  ...
  return {
    ...
    copyCIFiles: Boolean(options.copyCiFiles),
    force: Boolean(options.force),
    skipGit: Boolean(options.skipGit),
    electronVersion: options.electronVersion ?? 'latest',
    packageManager: options.packageManager ?? 'npm@latest',
  };
}

export function getImportOptions(argv: string[]): ParsedImportOptions {
  let dir: string;
  const program = createCommand();
  program
    .version(
  ...
  // Register the init subcommand so commander doesn't error on `init`
  program
    .command('init')
    .allowUnknownOption(true)
    .action(() => {});
...
export function createCLI(argv: string[]): void {
  const program = createCommand();
  program
    .version(
      packageJSON.version,
