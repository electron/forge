...
export const forgeImport = autoTrace(
  { name: 'import()', category: 'create-electron-app' },
  async (
    childTrace,
    {
      dir = process.cwd(),
      ...
      [
        {
          ...
          task: childTrace(
            { name: 'locate-project', category: 'create-electron-app' },
            async () => {
        ...
        },
        {
          ...
          task: childTrace<Parameters<ForgeListrTaskFn>>(
            { name: 'process-config', category: 'create-electron-app' },
            async (_, _ctx, task) => {
              ...
              const keys = Object.keys(packageJSON.dependencies).concat(
                Object.keys(packageJSON.devDependencies),
              );
              const buildToolPackages: Record<string, string | undefined> = {
                '@electron/get':
                  'already uses this module as a transitive dependency',
                '@electron/osx-sign':
                  'already uses this module as a transitive dependency',
                '@electron/packager':
                  'already uses this module as a transitive dependency',
              ...
              const updatePackageScript = async (
                scriptName: string,
                newValue: string,
              ) => {
              ...
              return task.newListr(
                [
                  ...
                  },
                  {
                    ...
                    task: async () => {
                      const pathToTemplateConfig = path.resolve(
                      ...
                      if (
                        ...
                        await fs.copyFile(
                          pathToTemplateConfig,
                          path.resolve(dir, 'forge.config.js'),
                        );
                      }
                    },
                  },
                  {
                    ...
                    task: async () => {
                      if (
                        await fs.pathExists(path.resolve(dir, '.gitignore'))
                      ) {
        ...
        },
        {
          ...
          task: childTrace<Parameters<ForgeListrTaskFn>>(
            { name: 'finalize-import', category: 'create-electron-app' },
            (_, __, task) => {
