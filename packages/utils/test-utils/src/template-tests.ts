    ...
    ['run', 'start'],
    {
      ...
      env: {
        ...
        ...(packageManager !== 'yarn' && {
           ...
           * that the project _also_ uses Yarn, so `electron-forge start` ends
           * up failing because there's no `yarn.lock` ([relevant code](https://github.com/electron/forge/blob/001f41befe2c049b6f54ce7d6c55e83435141055/packages/api/cli/src/util/check-system.ts#L108-L135)).
           *
           ...
           * validation and Forge treats it like an unsupported npm version, so
           * we also have to spoof a supported npm version number to work around
           * that behavior.
           */
