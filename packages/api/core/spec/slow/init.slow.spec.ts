...
describe('init', () => {
  let dir: string;
  const localForgePath = path.resolve(__dirname, '../../../../../');
  ...
  describe('with custom template', () => {
    it('adds all files correctly', async () => {
      // NOTE: this doesn't use the local Forge checkout to prevent conflicts with `portal:` deps
      await api.init({
        dir,
  ...
  describe('in an existing directory', () => {
    ...
    beforeAll(async () => {
      ...
      await api.init({ dir: persistentDir, localForgePath });
      return async () => {
