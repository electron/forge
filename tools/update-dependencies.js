...
class Package {
  ...
  async upgrade() {
    if (this.isMajorVersionBump() || this.isMinorVersionBump()) {
      await this.upgrade_with_packagejson_update();
    } else {
      await this.upgrade_without_packagejson_update();
    }
  }

  async upgrade_with_packagejson_update() {
  ...
  async upgrade_without_packagejson_update() {
